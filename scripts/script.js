...
const createStars = (rating) => {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    let star = createElementE(
      "span",
      { class: "material-symbols-outlined" },
      "star"
    );

    if (Math.ceil(rating) < i) star.classList.add("empty_star");
    else if (rating < i) star.innerText = "star_half";
...
const addCourse = (courseData) => {
  ...
  const instructorNameNode = createElementE(
    "span",
    { class: "utility-comment instructor" },
    courseData.instructors[0].name
  );

  const courseRatingNode = createElementE("div", {
    class: "rating-description",
  });
  const courseRatingInfoNode = createElementE(
    "span",
    { class: "rating" },
    courseRating
  );
  const starsContainerNode = createStars(courseRating);
  const coursePeopleAttendNode = createElementE(
    "span",
    { class: "utility-comment" },
    `(${coursePeopleAttend.toLocaleString()})`
  );
  ...
  const coursePriceInfoNode = createElementE("div", {
    class: "price-description",
  });
  const coursePriceNode = createElementE(
    "span",
    { class: "price" },
    `₦${courseData.price}`
  );
  const previousCoursePriceNode = createElementE(
    "span",
    { class: "previousPrice" },
    `₦${courseData.previousPrice}`
  );

  coursePriceInfoNode.appendChild(coursePriceNode);
  coursePriceInfoNode.appendChild(previousCoursePriceNode);
  ...
  courseNode.appendChild(coursePriceInfoNode);
...
const addCategory = (category) => {
  ...
  category.courses.forEach((course) =>
    coursesContainerNode.appendChild(addCourse(course))
  );
...
const toggleCourses = (radio) => {
  ...
  document
    .querySelectorAll(".course-content")
